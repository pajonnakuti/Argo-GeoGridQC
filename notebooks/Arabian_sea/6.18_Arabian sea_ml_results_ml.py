...
# ── Load data ─────────────────────────────────────────────────────────────────
CSV_PATH = r"D:\INCOIS\Agro_project\results\All_grids_models\all_grids_6model_results.csv"
...
# ── Colour scale helper ───────────────────────────────────────────────────────
def val_to_color(v, lo=0.15, hi=0.55):
...
HEADER = html.Div([
    html.Div([
        ...
        html.P("Arabian Sea · Gridwise · 6 Models · 2 Targets (temp_qc, psal_qc)",
...
def build_overview(fd, metric):
    ...
    model_avgs["color"] = model_avgs["model"].map(MODEL_COLORS)
    ...
    fig_avg.update_layout(showlegend=False, **DARK)
    dark_fig(fig_avg)
    ...
    fig_wins = px.pie(wc, names="model", values="wins", hole=0.55,
                      color="model", color_discrete_map=MODEL_COLORS)
    fig_wins.update_layout(showlegend=True, legend=dict(orientation="h",y=-0.15),**DARK)
    dark_fig(fig_wins)
    ...
    fig_time.update_layout(showlegend=False, **DARK)
    dark_fig(fig_time)
...
def build_models(fd, metric):
    ...
    fig_box.update_layout(showlegend=False, **DARK)
    dark_fig(fig_box)
    ...
    fig_gap.update_layout(title="Val F1 − Test F1 (positive = overfitting)",
...
def build_grids(fd, metric):
    ...
    fig_best.update_layout(title=f"Best {METRICS[metric]} per Grid",
                           xaxis_title="", yaxis_range=[0,1], showlegend=False, **DARK)
    dark_fig(fig_best)
    ...
    fig_rows.update_layout(showlegend=False, **DARK); dark_fig(fig_rows)
    ...
    fig_winner.update_layout(**DARK); dark_fig(fig_winner)
    ...
    fig_ht = px.imshow(pivot_wide, color_continuous_scale="RdYlGn",
                       ...
                       aspect="auto")
    fig_ht.update_layout(**DARK); dark_fig(fig_ht)
...
def build_heatmap(target):
    ...
    for t in tgts:
        ...
        fig.update_layout(**DARK, height=max(300, 40*len(pivot_wide)+100))
        dark_fig(fig)
